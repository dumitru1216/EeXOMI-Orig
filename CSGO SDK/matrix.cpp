...
void matrix3x4_t::TransformAABB( const Vector& vecMinsIn, const Vector& vecMaxsIn, Vector& vecMinsOut, Vector& vecMaxsOut ) const {
   ...
   Vector worldCenter = localCenter.Transform( *this );
   Vector worldMax = vecMaxsIn.Transform( *this );
   Vector worldMin = vecMinsIn.Transform( *this );

   vecMinsOut = ( worldCenter + ( worldCenter - worldMin ) * 2.0f );
   vecMaxsOut = ( worldCenter + ( worldMax - worldCenter ) * 2.0f );
#endif
}
