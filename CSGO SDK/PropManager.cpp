...
namespace Engine
{
  ...
  bool PropManager::Create( IBaseClientDLL* client ) {
	 if( !client ) {
		Win32::Error( XorStr( "IBaseClientDLL is nullptr (PropManager::%s)" ), __FUNCTION__ );
		return false;
	 }
	 ...
	 if( !entry ) {
		Win32::Error( XorStr( "ClientClass is nullptr (PropManager::%s)" ), __FUNCTION__ );
		return false;
	 }
	 ...
	 if( this->m_pTables.empty( ) ) {
		Win32::Error( XorStr( "std::vector<RecvTable*> is empty (PropManager::%s)" ), __FUNCTION__ );
		return false;
	 }
  ...
  RecvVarProxyFn PropManager::Hook( const RecvVarProxyFn hooked, const std::string& table_name, const std::string& prop_name ) {
	 ...
	 if( !GetProp( table_name, prop_name, &prop ) ) {
		Win32::Warning( XorStr( "Prop '%s::%s' not found (PropManager::%s)" ), table_name.c_str( ), prop_name.c_str( ), __FUNCTION__ );
		return nullptr;
	 }
  ...
  RecvTable* PropManager::GetTable( const std::string& name ) {
	 ...
	 Win32::Warning( XorStr( "Prop table '%s' not found (PropManager::%s)" ), name.c_str( ), __FUNCTION__ );
	 return nullptr;
  }
