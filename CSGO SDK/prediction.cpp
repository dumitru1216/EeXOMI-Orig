...
namespace Engine {
	...
	void Prediction::Begin( Encrypted_t<CUserCmd> _cmd, bool* send_packet ) {
		...
		*reinterpret_cast< CUserCmd** >( uint32_t( predictionData->m_pPlayer ) + 0x326C ) = predictionData->m_pCmd.Xor( ); // unk01
		...
		if ( predictionData->m_pCmd->impulse )
			*reinterpret_cast< uint32_t* >( uint32_t( predictionData->m_pPlayer ) + 0x31EC ) = predictionData->m_pCmd->impulse;

		/* actually the impulse thingy should be before updatebutton state, thats how csgo does it  */
		...
		// Source::m_pPrediction->CheckMovingGround( predictionData->m_pPlayer, Source::m_pGlobalVars->frametime );

		/*
			https://github.com/feifeigd/Counter-Strike-Global-Offensive/blob/master/cstrike15_src/game/client/prediction.cpp#L1090
			CheckMovingGround( player, gpGlobals->frametime );
		*/

#if 0
		// C_BasePlayer::SetLocalViewAngles : according to csgo src
		predictionData->m_pPlayer->pl( ).v_angle = _cmd->viewangles;
#endif
	...
	void Prediction::End( ) {
		...
		correct.tickbase_shift = g_TickbaseController.s_nExtraProcessingTicks;
